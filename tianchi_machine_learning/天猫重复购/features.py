...
from utils.df_related_process import reduce_mem_usage
...
def read_data(data_folder, num_rows=None, reduce_mem=True):
    train_file = os.path.join(data_folder, 'train_format1.csv')
    test_file = os.path.join(data_folder, 'test_format1.csv')

    user_info_file = os.path.join(data_folder, 'user_info_format1.csv')
    user_log_file = os.path.join(data_folder, 'user_log_format1.csv')

    train_data = read_csv(train_file, num_rows)
    test_data = read_csv(test_file, num_rows)

    user_info = read_csv(user_info_file, num_rows)
    user_log = read_csv(user_log_file, num_rows)
    
    if reduce_mem:
        for df in [train_data, test_data, user_info, user_log]:
            df = reduce_mem_usage(df)
        
    return train_data, test_data, user_info, user_log


train_data, test_data, user_info, user_log = read_data(data_folder)


# %%
# combine user info
all_data = train_data.append(test_data).merge(user_info, on = 'user_id', how='left')
...
# %%
import gensim
# model = Word2Vec(all_data_test['seller_path'].apply(lambda x: x.split(' ')), size=100, window=5, min_count=5, workers=56)
model = gensim.models.Word2Vec(all_data_test['seller_path'].apply(lambda x: x.split(' ')), window=5, min_count=5, workers=56)
